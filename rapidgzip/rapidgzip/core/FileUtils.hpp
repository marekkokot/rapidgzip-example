...
{
#ifdef _MSC_VER
	return make_unique_file_ptr(_fdopen(fileDescriptor, mode));
#else
...
#ifdef _MSC_VER
		const auto nBytesWritten = ::_write( outputFileDescriptor, currentBufferPosition, nBytesToWritePerCall );
#else
...
#ifdef _MSC_VER
                m_fileDescriptor = ::_fileno(m_outputFile.get());                
#else
